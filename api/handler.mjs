...
export default async function handler(req,res){res.setHeader('Cache-Control','no-store');res.setHeader('X-Content-Type-Options','nosniff');res.setHeader('Referrer-Policy','same-origin');try{
 ...
 if(req.method==='POST'&&path==='/api/weather/refresh')return send(res,200,await service.weather(input.test_wind_mph===undefined?undefined:Number(input.test_wind_mph)));
 ...
 }catch(error){console.error('Request failed:',req.method,req.url,error.code||error.name,String(error.message||'').slice(0,300));return send(res,error.status||error.statusCode||503,{error:error.status||error.statusCode?error.message:'Service temporarily unavailable; retry shortly. No successful write has been discarded.'})}}
