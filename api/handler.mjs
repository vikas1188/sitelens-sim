...
import {HttpError} from '../src/engine.mjs';
...
export default async function handler(req,res){res.setHeader('Cache-Control','no-store');res.setHeader('X-Content-Type-Options','nosniff');res.setHeader('Referrer-Policy','same-origin');try{
 ...
 const action=path.match(/^\/events\/([a-zA-Z0-9_.:-]+)\/(verdict|ack|correction)$/);if(req.method==='POST'&&action){const {event,state}=await service.action(action[1],action[2],input);return send(res,200,{...event,event,state_revision:state.revision})}
 if(req.method==='POST'&&['/api/detect','/detect/frame'].includes(path)){const detection=await queueDetection(input.image);const {event}=await service.ingest({event_id:randomUUID(),ts:new Date().toISOString(),source:'edge-worker-local-Liquid',type:'ppe_violation',zone:input.zone||'ZONE-A',detector_payload:detection});return send(res,200,{detection,detector_payload:detection,event})}
