export class ScenarioPlayer {
 constructor({physics,behaviors,onChange,onHazard}){Object.assign(this,{physics,behaviors,onChange,onHazard});this.state='ready';this.time=0;this.index=0;this.eventIndex=-1;this.completed=new Set();this.hazardMoment=null}
 select(scenario){this.physics.reset();this.scenario=scenario;this.time=0;this.eventIndex=-1;this.hazardMoment=null;this.state='ready';this.behavior=this.behaviors[scenario.id];this.behavior?.setup();this.onChange(this)}
 play(){if(this.state==='paused'){this.state='running'}else{this.select(this.scenario);this.state='running'}this.onChange(this)}
